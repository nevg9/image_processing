...
def train_model(net, dataloaders_dict, criterion, optimizer, num_epochs, global_step=0, start_epoch=0, log_dir=""):
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for phase in ['train', 'val']:
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    ...
                    if phase == 'train':
                        ...
                        if global_step % 1000 == 0:
                            ...
                            torch.save(checkpoint, log_dir + f"/model.ckpt-{global_step}")
            ...
            if phase == 'train':
                writer.add_scalar('train_loss_epoch', epoch_loss, global_step=global_step)
                writer.add_scalar('train_acc_epoch', epoch_acc, global_step=global_step)

            if phase == 'val':
                writer.add_scalar('val_loss_epoch', epoch_loss, global_step=global_step)
                writer.add_scalar('val_acc_epoch', epoch_acc, global_step=global_step)
...
def run_model(check_point_path, num_epochs, log_dir):
    ...
    accuracy_list, loss_list = train_model(
        net, dataloaders_dict, criterion, optimizer, num_epochs=num_epochs, global_step=global_step,
        start_epoch=start_epoch, log_dir=log_dir)


if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('-e', '--num_epoch', type=int, default=10, help='max epochs')
    parser.add_argument('-p', '--check_point_path', type=str, default='', help='check_point_path')
    parser.add_argument('-l', '--log_dir', type=str, default="", help="log dir")

    args = parser.parse_args()

    run_model(args.check_point_path, args.num_epoch, args.log_dir)
